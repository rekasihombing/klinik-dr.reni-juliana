...
public function index()
{
    try {
        ...
        // Modified query to sort by antrian
        ...
        $pasienMenungguKonfirmasi = Appointment::whereNotNull('checked_in_at')
            ...
            ->whereDate('tanggal', $today)
            ->orderBy('antrian', 'asc') // Optional: Sort pending confirmations by antrian
            ->get();
        ...
        \Log::info('Final data sent to frontend:', [
            ...
            'antrian_values' => $pasienHariIni->pluck('antrian')->toArray(), // Log queue numbers
        ]);
        ...
        return Inertia::render('staff/DashboardStaff', [
            ...
            'debug' => [
                ...
                'antrian_values' => $pasienHariIni->pluck('antrian')->toArray(), // Debug queue numbers
            ]
        ]);
    } catch (\Exception $e) {
...
private function getWeeklyActivity()
{
    ...
    $appointments = Appointment::whereBetween('tanggal', [$startOfWeek, $endOfWeek])
    ...
    for ($i = 0; $i < 7; $i++) {
        ...
        $statusCounts = [
            'dikonfirmasi' => $dayAppointments->where('status', 'dikonfirmasi')->count(),
            'diproses' => $dayAppointments->where('status', 'diproses')->count(),
            'selesai' => $dayAppointments->where('status', 'selesai')->count(),
        ];
        ...
        $weeklyData[] = [
            ...
            'label' => $date->locale('id')->isoFormat('DD MMM'),
        ];
    }
